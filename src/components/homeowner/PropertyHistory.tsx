...
const historyItems = [
  ...
  { icon: <AlertCircle className="w-4 h-4 text-era-navy/20" />, text: "Ingen kontrakter registrert", date: "" },
];
    ...
    <section className="py-20 px-6 bg-era-ivory/50 border-y border-era-navy/5">
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center gap-3 mb-12">
          <History className="w-5 h-5 text-era-navy/40" />
          ...
          {historyItems.map((item, i) => (
            ...
            >
              <div className="flex items-center gap-4">
                <div className="text-era-navy/30 group-hover:text-era-gold transition-colors">
                  {item.icon}
                </div>
                <span className="text-sm font-medium text-era-navy/80">{item.text}</span>
              </div>
              <span className="text-[10px] uppercase font-bold tracking-widest text-era-navy/30 italic">
                {item.date}
              </span>
            </motion.div>
          ))}
